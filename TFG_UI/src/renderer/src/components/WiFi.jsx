...
export default function Wifi() {
  ...
  const executeShellCommand = () => {
    ...
    ipcRenderer.send('get-bluetooth-devices', 'cat ./networks');
    ipcRenderer.on('command-result', (event, result) => {
      if (result.error) {
        ...
        setNetworks(result.stdout.split('\n'));
      }
    });
  };
  ...
  return (
    <section className="flex flex-col px-8">
      <h1 className="text-white text-3xl mt-5"> Wifi </h1>
      ...
      {active ?
        <>
          <div className="flex flex-col">
            <h2 className="text-white text-lg mt-5">Redes guardadas</h2>
            <div className='flex justify-center mt-10'>
              <p className="text-sm mt-5">No hay redes guardadas</p>
            </div>
          </div>
          <div className="flex flex-col">
            <h3 className="text-white text-lg mt-5">Redes disponibles</h3>
            <div className='flex flex-col mt-10'>
              {networks.length === 0 &&
                <div className="flex justify-center">
                  <p className="text-sm mt-5">No se encontraron redes disponibles</p>
                </div>
              }
              <div className="grid grid-cols-4">
                {networks.map((device, index) => {
                  return (
        ...
        <div className="flex justify-center items-center h-64">
          <p>Activa la función Wifi para escanear redes cercanas</p>
        </div>
      }
    </section>
  )
